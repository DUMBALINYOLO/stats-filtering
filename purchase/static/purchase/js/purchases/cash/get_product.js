...
var get_product_form = {
	...
	elements: [
		...
		{
			view: "counter", step:1,min:1, label: 'CP ()', name: "cp", on: {
				onEnter: function () {
		...
		{
			view: "counter",step:1,min:1, label: 'SP ()', name: "sp", on: {
				onEnter: function () {
						...
						onChange: function () {
							if ($$("get_product").getValue() == "") {
								...
								webix.ajax().post(uri, function (text, response) {
									...
									data = response.json();
									// console.log(data);
									datatable = $$("purchase_datatable").getItem(data.id);
									// console.log(datatable);
									if (data.status) {
										if (datatable) {
											$$("purchase_datatable").editCell(data.id, "order_qty", false, true);
										} else {
